...
int map_getPath(char sMap[50], int xS, int yS, int xE, int yE, File * path);
void map_readMap(char sMap[50], pathMap * pm);
void map_putFrontier(pathMap * cPathMap, frontier * frontier, int x, int y);
pos2D map_getFrontier(frontier * f);
...
int map_getPath(char sMap[50], int xS, int yS, int xE, int yE, File * path)
{
    pathMap * cPathMap = (pathMap*)malloc(sizeof(pathMap));
    int width = getMapWidth(sMap);
    int height = getMapHeight(sMap);
    if(xS < 0 || xS > width ||
       yS < 0 || yS > height)
    {
    ...
    {
        pos2D current = map_getFrontier(&frt);
...
pos2D map_getFrontier(frontier * frt)
{
    pos2D p = frt->coord[0];
    frt->taille--;
